...
def plot_individual_amplitude_with_fit(ax: Axes, ds: xr.Dataset, qubit: dict[str, str], fit: xr.Dataset = None):
    ...
    def _fit_curves(fit_ds: xr.Dataset):
        """Return list of (label, fitted_data) for each TWPA frequency (or single fit)."""
        if fit_ds is None:
            return []
        dim = "TWPA_freq"
        if dim in fit_ds.dims:
            curves = []
            for twpa_freq in fit_ds[dim].values:
                params = fit_ds.sel(**{dim: twpa_freq})
                curves.append(
                    (
                        f"fit {twpa_freq/1e6:.2f} MHz",
                        lorentzian_dip(
                            ds.detuning,
    ...
    (ds.assign_coords(full_freq_GHz=ds.full_freq / u.GHz).loc[qubit].IQ_abs / u.mV).plot.line(
        ax=ax, x="full_freq_GHz", hue="TWPA_freq", add_legend=False
    )
    ...
    (ds.assign_coords(detuning_MHz=ds.detuning / u.MHz).loc[qubit].IQ_abs / u.mV).plot.line(
        ax=ax2, x="detuning_MHz", hue="TWPA_freq", add_legend=False
    )
    ax2.set_xlabel("Detuning [MHz]")
