...
node = QualibrationNode(name="02a_Resonator_Spectroscopy_TWPA_gain", parameters=Parameters())
assert not (
...
if node.parameters.simulate:
    ...
    job_ = []
    for i in gain_sweep:
        open_TWPA(addr=addr,power=True,pump_frequency=TWPA_fre,gain=i)
        time.sleep(2)
...
if not node.parameters.simulate:
    ...
    if node.parameters.load_data_id is not None:
        ...
        for i in range(len(gain_sweep)):
            ...
            ds = xr.concat([ds, ds_], dim="gain") if i != 0 else ds_
        ds = ds.assign_coords(gain=gain_sweep)
    ...
    for ax, qubit in grid_iter(grid):
        for i in range(len(gain_sweep)):
            (ds.sel(gain=gain_sweep[i]).assign_coords(freq_MHz=ds.freq / 1e6).loc[qubit].IQ_abs * 1e3).plot(ax=ax, x="freq_MHz",label=f"{gain_sweep[i]:.2}")
    ...
    for ax, qubit in grid_iter(grid):
        for i in range(len(gain_sweep)):
            (ds.sel(gain=gain_sweep[i]).assign_coords(freq_MHz=ds.freq / 1e6).loc[qubit].phase * 1e3).plot(ax=ax, x="freq_MHz")
        ax.set_xlabel("Resonator detuning [MHz]")
