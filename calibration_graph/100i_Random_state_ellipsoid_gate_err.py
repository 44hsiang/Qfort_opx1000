...
if Parameters().load_data_id is not None:
    ...
    node = QualibrationNode(name="100i_Random_state_ellipsoid_gate_err", parameters=Parameters())
        ...
        with for_(n, 0, n < n_runs, n + 1):
            ...
            with for_each_((theta, phi),(theta_list, phi_list)):
                with for_(tomo_axis, 0, tomo_axis < 3, tomo_axis + 1):
                    ...
                    qubit.align()
                    qubit.xy.play("y180",amplitude_scale = theta/np.pi * alpha)
...
if not node.parameters.simulate:
    if load_data_id is None:
        ds = fetch_results_as_xarray(job.result_handles, [qubits[0]], { "axis": ['x','y','z'],"n_points": np.arange(n_points)})
    else:
    ...
    print(f"param: {param}")
    print(f"memory robustness: {QuantumMemory.memory_robustness(choi)})")
