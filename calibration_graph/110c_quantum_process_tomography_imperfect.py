...
alpha_list = np.arange(0.01, 2.01, 0.01)
for a in alpha_list:
    # %% {Node_parameters}
    ...
    node = QualibrationNode(name="110c_quantum_process_tomography_imperfect", parameters=Parameters())
    ...
    def QuantumMemory_program(qubit, alpha):
                ...
                with for_(initial_state, 0, initial_state < 4, initial_state + 1):
                    ...
                    with if_(initial_state == 0):
                        ...
                        with for_(tomo_axis, 0, tomo_axis < 3, tomo_axis + 1):
                            ...
                            wait(10)
                            #initial state |0|
                            if operation_name != "id": qubit.xy.play(operation_name, amplitude_scale=alpha)
                            qubit.align()
                            wait(delay_time)
                            #tomography pulses
    ...
    if node.parameters.simulate:
        ...
        job = qmm.simulate(config, QuantumMemory_program(qubits[0], alpha), simulation_config)
        ...
        for qubit in qubits:
            with qm_session(qmm, config, timeout=node.parameters.timeout) as qm:
                job = qm.execute(QuantumMemory_program(qubit, alpha))
    ...
    if not node.parameters.simulate:
        ...
        for i,initial_state in enumerate(ds.initial_state.values):
            ...
            for ax, qubit in grid_iter(grid):
                ...
                ax.set_title(qubit['qubit'] + f" - amp: {alpha}")
                ...
                bloch.render(title=qubit['qubit'] + '_'+initial_state + f" - amp: {alpha}")
