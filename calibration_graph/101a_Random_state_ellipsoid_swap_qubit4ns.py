...
class Parameters(NodeParameters):
    ...
    timeout: int = 100
    load_data_id: Optional[int] = 1683
...
if Parameters().load_data_id is not None:
    ...
    node = QualibrationNode(name=f"101a_Random_state_ellipsoid_swap_qubit4ns_{Parameters().interaction_time_ns}", parameters=Parameters())
...
interaction_time = node.parameters.interaction_time_ns//4
    ...
    with program() as QuantumMemory:
        ...
        with for_(n, 0, n < n_runs, n + 1):
            ...
            with for_(tomo_axis, 0, tomo_axis < 3, tomo_axis + 1):
                ...
                align()
                noise_qubit.z.play('const',duration=interaction_time,amplitude_scale=iswap_point/noise_qubit.z.operations['const'].amplitude)            
                align()
                wait(interaction_time+13)
                #tomography pulses
