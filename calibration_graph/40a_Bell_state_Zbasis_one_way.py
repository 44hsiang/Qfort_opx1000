...
class Parameters(NodeParameters):
    ...
    qubit_triplets: List[List[str]] = [["q3","q2","q3"]]# list of lists of the qubits making up the GHZ state
    num_shots: int = 5000
    flux_point_joint_or_independent: Literal["joint", "independent"] = "joint"
    reset_type: Literal['active', 'thermal'] = "thermal"
    simulate: bool = False
    timeout: int = 100
    load_data_id: Optional[int] = 2003


node = QualibrationNode(
    name="40a_Bell_state_Zbasis_one_way", parameters=Parameters()
)
...
#%% delete core
for i,j in zip(machine.active_qubit_names,"aaaaa"):
...
with program() as CPhase_Oscillations:
    ...
    for i, qubit_triplet in enumerate(qubit_triplets):
        ...
        with for_(n, 0, n < n_shots, n + 1):
            ...
            align()'''
            align()
            qubit_triplet.qubit_A.xy.play("x180")
            qubit_triplet.qubit_B.xy.play("x180")

            align()
            readout_state(qubit_triplet.qubit_A, state_A[i])
            save(state_A[i], state_st_A[i])
            active_reset(qubit_triplet.qubit_A, "readout",max_attempts=2,wait_time=100)
            '''align()
            qubit_triplet.qubit_B.xy.play("y90")
            ...
            align()'''
            align()
            qubit_triplet.qubit_C.xy.play("x180")
            align()
            #readout_state(qubit_triplet.qubit_A, state_A[i])
