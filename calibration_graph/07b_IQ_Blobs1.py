...
node = QualibrationNode(name="07b_IQ_Blobs1", parameters=Parameters())
...
def iq_blobs_program(qubit):
    with program() as iq_blobs:
        ...
        with stream_processing():
            n_st.save("n")
            I_g_st[0].save_all(f"I_g1")
            Q_g_st[0].save_all(f"Q_g1")
            I_e_st[0].save_all(f"I_e1")
            Q_e_st[0].save_all(f"Q_e1")
    return iq_blobs
...
if node.parameters.simulate:
    ...
    for qubit in qubits:
        with qm_session(qmm, config, timeout=node.parameters.timeout) as qm:
            job = qm.execute(iq_blobs_program(qubit))
