...
from calibration_utils.TWPA_gain import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
...
from instrument.TWPA_pump import open_TWPA, TWPAInfo
# %% {Initialisation}
description = """
...
node = QualibrationNode[Parameters, Quam](
    name="02a1_resonator_spectroscopy_TWPA_gain",  # Name should be unique
...
def execute_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    with qm_session(qmm, config, timeout=node.parameters.timeout) as qm:
        ...
        for i, pump_power in enumerate(pump_steps):
            open_TWPA(addr=node.parameters.TWPA_address, power=True, pump_frequency=pump_frequency, gain=pump_power)
            TWPA_info = TWPAInfo(addr=node.parameters.TWPA_address)
            print(f"TWPA pump power set to {TWPA_info.power} dBm")
            ...
            print(f"Job execution {i+1}/{len(pump_steps)} completed.")
            datasets = [] if i == 0 else datasets
            datasets.append(dataset)
            # Display the execution report to expose possible runtime errors
            node.log(job.execution_report())
        
        # Register the raw dataset
        concat_dataset = xr.concat(datasets,dim=xr.IndexVariable("TWPA_amp", pump_steps))
    #node.results['ds_raws'] = datasets
    node.results["ds_raw"] = concat_dataset
