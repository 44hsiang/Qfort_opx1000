...
class TWPAInfo:
    ...
    def __init__(self, addr: str):
        ...
        self.rf_output = instr.query_bool('OUTPUT?')
        self.power = instr.query_float('POWER?')
        self.frequency = instr.query_float('FREQUENCY?')

    @classmethod
    def from_address(cls, addr: str) -> "TWPAInfo":
        return cls(addr)

def TWPA_info(addr: str) -> TWPAInfo:
    return TWPAInfo(addr)

if __name__== '__main__':
    addr = 'TCPIP::192.168.50.12::INSTR'
    info = TWPAInfo(addr)
    print(f"TWPA out is {info.rf_output}")
    print(f"TWPA poser is {info.power}")
    print(f"TWPA frequecny is {info.frequency}")
